...
class HT_Model_administrator_models_useradmin extends Zend_Db_Table {//ten class fai viet hoa
	...
	private function _getUserInfoByUsername($username){
		$sql = " SELECT * FROM user WHERE user_name = ? AND active = 1 LIMIT 1";
	...
	function addData($data,$tableName=null){
		if($tableName) $this->_name = $tableName;
		$this->insert($data);
		if($tableName) $this->_name = "user";
		if(!$tableName){
	...
	public function getUser_id($userid,$filter = array()) {
		$sql = " SELECT user_name,firstname,lastname,birthday,email,pass FROM user WHERE userid= ".(int)$userid;
		return $this->_db->fetchRow($sql);
	}
	
	public function getUser_pass_id($userid,$filter = array()) {
		$sql = " SELECT user_name,firstname,lastname,birthday,email,pass FROM user WHERE userid= ".(int)$userid;
		return $this->_db->fetchRow($sql);
	}
